...
#       [--channel glm|glm-thinking|custom|local] [--model M] \
#       [--base-url U] [--api-key K] [--json] [--timeout N]
# Env: GLM_API_KEY, VISION_CUSTOM_API_KEY, VISION_CUSTOM_BASE_URL,
#      VISION_CUSTOM_MODEL, LOCAL_VLM_URL, LOCAL_VLM_MODEL
# ============================================================
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: vlm-vision.sh --image <path|url> [--prompt text] [--channel glm|glm-thinking|custom|local] [--model M] [--base-url U] [--api-key K] [--json] [--timeout N]"
            exit 0 ;;
...
case "$CHANNEL" in
    ...
    local)
        ...
        [ -z "$MODEL" ]    && MODEL="${LOCAL_VLM_MODEL:-llama3.2-vision}"
        ;;
